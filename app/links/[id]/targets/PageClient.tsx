...
export default function TargetsPage({ user, id }: TargetsPageProps) {
  ...
  // Since authentication is handled in default.tsx, we can skip client-side check
  useEffect(() => {
  ...
  // Inject template params into utm_overrides when template changes
  useEffect(() => {
    if (
      draft.utm_template_id &&
      templatePreview &&
      Object.keys(templatePreview).length > 0
    ) {
      const currentOverrides = draft.rules?.utm_overrides || {};
      const merged = { ...templatePreview, ...currentOverrides };

      dispatch({
        type: "merge_rules",
        value: { utm_overrides: merged },
      });
    }
  ...
  // Compute final UTM params (template + overrides)
  const finalUtmParams = useMemo(() => {
    return draft.rules?.utm_overrides || {};
      ...
      <form onSubmit={saveDraft} className="p-4 rounded border mb-6 space-y-4">
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <label className="block text-sm">UTM Template</label>
            <Button
              type="button"
              variant="outline"
              size="sm"
          ...
          <MultiSelect
            ...
            value={draft.utm_template_id ? [String(draft.utm_template_id)] : []}
            onValueChange={(vals: string[]) => {
              const selectedId = vals.length ? Number(vals[0]) : null;
              dispatch({
                type: "set_field",
                key: "utm_template_id",
                value: selectedId,
        ...
        {/* Template Preview - show inherited params */}
        {draft.utm_template_id && Object.keys(templatePreview).length > 0 && (
          <div className="mt-4 p-4 bg-blue-50 rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              {Object.entries(templatePreview)
                .filter(([_, v]) => v)
                .map(([key, value]) => (
                  <div key={key} className="p-2 rounded border bg-white">
        ...
        {Object.keys(finalUtmParams).length > 0 && (
          <div className="py-2">
            <label className="block text-xs text-muted-foreground mb-1 font-semibold">
              Final UTM Params (inherited + overrides)
            </label>
            <pre className="border rounded p-2 text-xs overflow-x-auto bg-muted">
              {JSON.stringify(finalUtmParams, null, 2)}
            </pre>
          </div>
        )}
