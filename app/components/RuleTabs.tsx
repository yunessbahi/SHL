...
export default function RuleTabs({ rules, setRules }: { rules: any; setRules: (r: any) => void }) {
	...
	return (
		<div className="space-y-4">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
					...
					<div>
						...
						<select
							...
							onChange={(e) => setRules({ ...rules, device_allow: e.target.value ? [e.target.value] : [] })}
						>
			...
			{tab === "behavior" && (
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
					<DateTimePicker
						...
						onChange={(v) => setRules({ ...rules, time_window: { ...(rules?.time_window || {}), start: v } })}
					/>
					<DateTimePicker
						label="End (ISO)"
						value={rules?.time_window?.end || ""}
						onChange={(v) => setRules({ ...rules, time_window: { ...(rules?.time_window || {}), end: v } })}
					/>
			...
			{tab === "json" && (
				<JsonEditor value={jsonStr} onChange={(s) => { try { setRules(JSON.parse(s)); } catch {} }} />
			)}
		</div>
	);
}
