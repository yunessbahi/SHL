...
export default function LinksDataTable({
  ...
  // Expanded row data - separate state to avoid pagination reset
  const [expandedLinksData, setExpandedLinksData] = useState<
    Map<string, Partial<IData>>
  >(new Map());
  ...
  // Merged data combining base linksData with expanded data - used only for filtering
  const mergedData = useMemo(() => {
    return linksData.map((link) => {
      const expandedData = expandedLinksData.get(link.id);
      return expandedData ? { ...link, ...expandedData } : link;
    });
  }, [linksData, expandedLinksData]);
    ...
    async (linkId: string) => {
      ...
      try {
        ...
        if (detailedData) {
          ...
          // Update the expanded links data with real counts and details
          setExpandedLinksData((prev) => {
  ...
  useEffect(() => {
    if (utmTemplates.length > 0 && expandedDataCache.size > 0) {
      ...
      setExpandedLinksData((prev) => {
        const newMap = new Map(prev);
        expandedDataCache.forEach((expandedData, linkId) => {
          if (expandedData && expandedData.targets) {
            console.log(
              `🔄 Reprocessing link ${linkId} targets with ${utmTemplates.length} templates`,
            );
            ...
            const existingData = newMap.get(linkId) || {};
            newMap.set(linkId, {
              ...existingData,
  ...
  const applyFiltersToData = useCallback(
    (newFilters: Filter[], data: IData[] = mergedData) => {
      let filtered = [...data];
  ...
  // Initial data load - populate filteredData immediately when mergedData is available
  useEffect(() => {
    if (mergedData.length > 0) {
      const filtered = applyFiltersToData(filters);
      setFilteredData(filtered);
    } else if (mergedData.length === 0 && !loading) {
      // Clear filteredData when no mergedData is available
      setFilteredData([]);
    }
  }, [mergedData, applyFiltersToData, filters, loading]);
    ...
    () => [
      {
        ...
        meta: {
          expandedContent: (row) => {
            ...
            const expandedData = expandedLinksData.get(linkId);
            return (
                        ...
                        <Table className="text-xs w-full text-balance">
                          <TableBody>
                            ...
                            <TableRow className="*:border-border hover:bg-transparent [&>:not(:last-child)]:border-r">
                              ...
                              <TableCell className="py-2">
                                {expandedData?.fallback_url ? (
                                  <a
                                    href={expandedData.fallback_url}
                                    className="text-blue-600 hover:underline"
                                  >
                                    {expandedData.fallback_url}
                                  </a>
                          ...
                          <TableBody>
                            <TableRow className="*:border-border hover:bg-transparent [&>:not(:last-child)]:border-r">
                              ...
                              <TableCell className="flex py-2">
                                {expandedData?.rules?.ip_addresses &&
                                expandedData.rules.ip_addresses.length > 0 ? (
                                  expandedData.rules.ip_addresses.length ===
                                  1 ? (
                                    expandedData.rules.ip_addresses[0]
                                  ) : (
                                    expandedData.rules.ip_addresses.join(", ")
                                  )
                                ) : (
                            ...
                            <TableRow className="*:border-border hover:bg-transparent [&>:not(:last-child)]:border-r">
                              ...
                              <TableCell className="py-2">
                                {expandedData?.rules?.referer_domains &&
                                expandedData.rules.referer_domains.length >
                                  0 ? (
                                  expandedData.rules.referer_domains.length ===
                                  1 ? (
                                    expandedData.rules.referer_domains[0]
                                  ) : (
                                    expandedData.rules.referer_domains.join(
                                      ", ",
                                    )
                                  )
                                ) : (
                            ...
                            <TableRow className="*:border-border hover:bg-transparent [&>:not(:last-child)]:border-r">
                              ...
                              <TableCell className="py-2">
                                {row.expires_at && (
                                  <div className="w-full">
                                    <ProgressTTL
                                      startDate={expandedData?.start_at}
      ...
      },
      {
        ...
        header: "Targets",
        cell: ({ row }) => {
          // Compute targets count from expanded data if available, otherwise use base count
          const expandedData = expandedLinksData.get(row.original.id);
          const targetsCount =
            expandedData?.targets_count ?? row.original.targets_count;
          return <span className="font-normal text-sm">{targetsCount}</span>;
        },
        size: 100,
        enableSorting: true,
        meta: {
