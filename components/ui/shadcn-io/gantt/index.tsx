...
export type GanttFeature = {
  ...
  backgroundClass?: string; // Optional: custom background class for the feature bar
};
...
export const GanttSidebarGroup: FC<GanttSidebarGroupProps> = ({
  ...
  <div className={className}>
    <div
      className="flex content-center w-full p-2.5 text-left font-medium bg-muted-foreground/5 text-muted-foreground text-xs"
      style={{ height: "var(--gantt-row-height)" }}
    >
      {nameRenderer ? nameRenderer(name) : <p className="truncate">{name}</p>}
    </div>
...
export const GanttFeatureItemCard: FC<GanttFeatureItemCardProps> = ({
  ...
  return (
    <Card
      className={cn(
        "h-8 w-full rounded-md px-2 text-xs shadow-sm",
...
export const GanttFeatureItem: FC<GanttFeatureItemProps> = ({
  ...
  return (
    ...
    >
      <div
        className="pointer-events-auto absolute content-center"
        style={{
          height: "calc(var(--gantt-row-height) - 2px)",
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  useEffect(() => {
    if (scrollRef.current) {
      // For schedules page, scroll to today's date instead of center
      const isSchedulesPage = window.location.pathname.includes("/schedules");
      if (isSchedulesPage) {
        ...
        if (range === "monthly") {
          // For monthly view, scroll to current month in current year
          const baseColumnWidth = 150;
          const columnWidth = (baseColumnWidth * zoom) / 100;
          // Find which year index in timelineData contains current year
          const currentYearIndex = timelineData.findIndex(
            (yearData) => yearData.year === currentYear,
          );
          const monthsBeforeCurrentYear = currentYearIndex * 12; // 12 months per year
          scrollPosition =
            (monthsBeforeCurrentYear + currentMonth) * columnWidth;
        } else if (range === "daily") {
          // For daily view, calculate days since timeline start (previous year)
          const timelineStartDate = new Date(today.getFullYear() - 1, 0, 1); // Jan 1 of previous year
          const daysSinceTimelineStart = Math.floor(
            (today.getTime() - timelineStartDate.getTime()) /
              (1000 * 60 * 60 * 24),
          );
          const baseColumnWidth = 50;
          const columnWidth = (baseColumnWidth * zoom) / 100;
          scrollPosition = daysSinceTimelineStart * columnWidth;
        } else {
          // For quarterly or other views, use center
          scrollPosition =
            scrollRef.current.scrollWidth / 2 -
            scrollRef.current.clientWidth / 2;
        }

        // For schedules page, don't center - just scroll to make today's date visible at the left edge
        // This ensures today's date is visible without being pushed off-screen by centering
